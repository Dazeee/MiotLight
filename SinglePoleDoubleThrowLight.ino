...
const int SERVO_DELAY_MILLS = 1000; // 舵机归位延时时间

//创建一个Bool类型的变量
bool oState = false, servoState = false;
...
void reverseServo() {
    BLINKER_LOG("reverseServo servoState: ", servoState, ", oState: ", oState);
    if (servoState) {
    ...
    Blinker.delay(SERVO_DELAY_MILLS);
...
void dataRead(const String & data)
{
  ...
  Blinker.print("servoState=", servoState);
  Blinker.print("oState=", oState);
}
...
void setup() {
  ...
  Button1.attach(button_callback); //绑定按键执行回调函数
  Button2.attach(button_callback); //绑定按键执行回调函数
